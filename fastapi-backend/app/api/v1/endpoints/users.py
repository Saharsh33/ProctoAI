...
def list_users(
    ...
    logger.info("Listing users: skip=%d, limit=%d, requested_by=%s", skip, limit, current_user.user_id)
...
def create_user(
    ...
    logger.info("Creating user: email=%s, role=%s, requested_by=%s", payload.email, payload.role, current_user.user_id)
    existing = crud.get_user_by_email(db, str(payload.email))
    if existing:
        logger.warning("User creation rejected — email already exists: %s", payload.email)
