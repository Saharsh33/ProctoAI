...
def create(db: Session, log_in: ProctoringLogCreate) -> ProctoringLog:
    ...
    db.commit()
    db.refresh(log)
    logger.info("Proctoring log created: id=%s, email=%s, test_id=%s", log.lid, log.email, log.test_id)
    return log
...
def list_logs(db: Session, email: str | None = None, test_id: str | None = None, skip: int = 0, limit: int = 100) -> list[ProctoringLog]:
    ...
    logger.debug("Listed %d proctoring logs (email=%s, test_id=%s)", len(logs), email, test_id)
    return logs
